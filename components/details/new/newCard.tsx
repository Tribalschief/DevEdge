...
export const IndustryCard = ({ number, title, description, backgroundIcon }: IndustryCardProps) => {
  ...
  // Render a preview of the description with line clamp
  const renderPreviewDescription = (desc: string) => {
    // Get the first sentence or a portion of text
    const firstSentence = desc.split(/\.\s/)[0] + "."

    return (
      <div className="text-gray-800 text-xs sm:text-sm leading-relaxed drop-shadow-sm">
        <p className="line-clamp-3">{firstSentence}</p>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute z-20 -top-8 sm:-top-9 md:-top-10 left-1/2 transform -translate-x-1/2">
          <div
            className="relative bg-white rounded-full border border-gray-200 shadow-lg
                      w-16 h-16 sm:w-18 sm:h-18 md:w-20 md:h-20
                      flex items-center justify-center
                      overflow-hidden
                      transition-all duration-300 hover:shadow-xl"
          >
            {backgroundIcon ? (
              ...
              <div className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 bg-gray-100 rounded-full flex items-center justify-center">
                <span className="text-gray-400 text-xl">{title ? title.charAt(0).toUpperCase() : "?"}</span>
              </div>
            )}
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            {/* Description preview */}
            <div className="flex-grow">{renderPreviewDescription(description)}</div>

            {/* See More Button */}
            <button
              onClick={() => setIsModalOpen(true)}
              className="mt-3 text-xs sm:text-sm text-purple-600 hover:text-purple-800 font-medium transition-colors"
            >
              See More
            </button>
          </div>
