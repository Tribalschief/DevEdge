  ...
  <div>
    ...
    <div className="space-y-2 mt-2">
      {leader.background.map((paragraph, index) => (
        ...
        <p key={index} className="text-xs sm:text-sm text-justify">
          {paragraph}
        </p>
      ))}
    </div>
    ...
    <ul className="text-xs sm:text-sm mt-2 space-y-4 sm:space-y-6">
      {leader.engagements.map((engagement, index) => (
        <li key={index}>
          ...
          {/* --- Added text-justify here --- */}
          <p className="ml-4 text-justify">{engagement.description}</p>
        </li>
      ))}
    </ul>
  </div>
